...
module.exports.run = async ({ args, api, event, Currencies, client }) => {
   ...
   if (args.length == 0) return api.sendMessage(` ===  Bạn có thể dùng  === \n--------\n[☭] => Bot ndfb => Lọc thành viên bị bay acc khỏi nhóm\n--------\n[☭] => Bot del => Lọc thành viên khỏi nhóm theo số tin nhắn\n--------\n[☭] => Bot onl => Xem thời gian hoạt động bot onl \n--------\n[☭] => Bot tt => Check độ tương tác của bạn\n--------\n[☭] => Bot box => Lọc nhóm dưới 4 thành viên\n--------\n[☭] => Bot all => Check tất cả độ tương tác các thành viên trong box\n--------\n[☭] => Bot covid => Xem thông tin covid\n--------\n[☭] => Bot mayman => Xem Tỉ lệ % may mắn của bạn?\n--------\n[☭] => Bot nude => Check những ảnh gợi cảm\n--------\n    === 「${timeNow}」 ===`, event.threadID, event.messageID);
    var arr = [];
    ...
    if (!find) return api.sendMessage(`[卍] => Bạn và bot cần là quản trị viên!`,event.threadID);
      let storage = [];
    ...
    const find = data.adminIDs.find(el => el.id == event.senderID && api.getCurrentUserID());
    if (!find) return api.sendMessage(`[卍] => Bạn và bot cần là quản trị viên!`,event.threadID);
    if (!args[1]) return api.sendMessage(`[卍] => HDSD: check del => số tin nhắn cần lọc `,event.threadID);
      let storage = [];
  ...
  return api.sendMessage(
    ...
    event.threadID, event.messageID
      ...
      if (event.senderID != 100013942628281) return api.sendMessage(`Bạn đâu phải admin =))`, event.threadID, event.messageID)
            let number = [];
           ...
           return api.sendMessage(`[⚙️] => Đang tiến hành lọc những nhóm không tên và dưới 0 thành viên.`,threadID)
    }
    ...
    setTimeout(() => {
      api.sendMessage(`[卍] => Ping: ${(Date.now() - timeStart)}ms \n[卍] => Thời gian bot hoạt động: ${hours}:${minutes}:${seconds}`, event.threadID, event.messageID);
    }, 200);
  }, event.messageID);
